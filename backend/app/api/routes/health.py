...
def build_health_snapshot(app: FastAPI) -> dict[str, Any]:
    """Everything /api/health reports, read off app.state -- pulled out so
    /metrics (see routes/metrics.py) can expose the same numbers in
    Prometheus format from the same single source of truth instead of a
    second, independently-drifting set of getattr(...) reads."""
    tailers = getattr(app.state, "log_tailers", {})
    ...
    retention_job = getattr(app.state, "retention_job", None)
...
async def health(request: Request, strict: bool = False) -> dict | JSONResponse:
    ...
    matters most.
    """
    payload = build_health_snapshot(request.app)
    ...
    is_unhealthy = not payload["log_tailer_alive"] or payload["aggregator_events_likely_lost"]
