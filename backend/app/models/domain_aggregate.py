...
class DomainMinuteAggregate(Base):
    ...
    __table_args__ = (
        ...
        # upsert relies on at most one row per (bucket_ts, domain, branch)
        # existing. A future multi-instance aggregator or a duplicate flush
        # would otherwise silently create a second row and split/undercount
        # stats instead of failing loudly. branch joined the key so the same
        # domain in two different branches' logs in the same minute doesn't
        # collide into one row.
        Index("ix_domain_bucket_domain", "bucket_ts", "domain", "branch", unique=True),
    )
