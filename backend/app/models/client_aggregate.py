...
class ClientMinuteAggregate(Base):
    ...
    (bucket_ts, client_ip, coalesce(user, '')) is a *expression* index, which
    SQLAlchemy's declarative __table_args__ can't reference cleanly before
    the columns exist as bound attributes -- it's created via raw DDL in the
    `client_minute_aggregate_unique` migration instead (see
