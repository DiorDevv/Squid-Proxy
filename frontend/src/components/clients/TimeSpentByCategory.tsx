...
import { useTimeSpentByCategory } from '@/hooks/useTimeSpent'
import { useTranslation } from '@/i18n'

interface TimeSpentByCategoryProps {
  clientIp: string | undefined
  rangeParams: Record<string, string>
}

export function TimeSpentByCategory({ clientIp, rangeParams }: TimeSpentByCategoryProps) {
  const { t } = useTranslation()
  const query = useTimeSpentByCategory(clientIp, rangeParams)
  ...
  return (
    <ol className="scrollbar-thin -mr-1 flex h-[30rem] flex-col divide-y divide-border overflow-y-auto pr-1">
      {items.map((item) => (
        <li key={item.category} className="flex flex-col gap-1 rounded-md px-1 py-2.5 text-sm">
          <div className="flex items-center gap-3">
            <span className="min-w-0 flex-1 truncate text-foreground">{t(CATEGORY_LABEL_KEYS[item.category])}</span>
