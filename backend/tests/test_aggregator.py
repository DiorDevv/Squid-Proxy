...
def two_size_denied_line(domain: str, recv: int, sent: int, client_ip: str = "10.0.0.5") -> str:
    # Same shape as two_size_line() (GET, same URL format) but TCP_DENIED --
    # a blocked request, same domain/user ("alice") so it lands in the same
    # domain/client bucket as an allowed line for the same domain.
    return (
        f"1737100800.123 45 {client_ip} TCP_DENIED/403 {recv} {sent} GET "
        f"http://{domain}/ alice HIER_DIRECT/1.2.3.4 text/html"
    )


async def test_flush_splits_client_blocked_bytes_from_allowed(db_engine, monkeypatch):
    # Regression for a real bug report: a blocked request's bytes (a denial
    # page's size) were folding into the client's Downloaded/Uploaded
    # totals. total_bytes/bytes_received must reflect allowed traffic only;
    # blocked_bytes/blocked_bytes_received carry the excluded slice.
    session_factory = async_sessionmaker(db_engine, expire_on_commit=False, class_=AsyncSession)
    import app.services.aggregator as aggregator_module
    ...
    ring_buffer = RingBuffer(max_events=100)
    ring_buffer.append(parse_line(two_size_line("example.com", recv=900, sent=100)))  # allowed
    ring_buffer.append(parse_line(two_size_denied_line("example.com", recv=50, sent=20)))  # blocked

    await Aggregator(ring_buffer=ring_buffer, interval_seconds=60).flush()

    async with session_factory() as session:
        client_row = (await session.execute(select(ClientMinuteAggregate))).scalar_one()
        assert client_row.total_bytes == 100  # the allowed line's %<st only
        assert client_row.bytes_received == 900  # the allowed line's %>st only
        assert client_row.blocked_bytes == 20  # the denied line's %<st
        assert client_row.blocked_bytes_received == 50  # the denied line's %>st

        domain_row = (await session.execute(select(DomainMinuteAggregate))).scalar_one()
        assert domain_row.total_bytes == 100
        assert domain_row.bytes_received == 900
